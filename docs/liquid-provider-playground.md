...
The suite builds and loads the real unpacked extension. It verifies the safe public checks, seeds a deterministic testnet wallet through an extension-only message, drives Apogee's real `getBalance`, `getUTXOs`, `getWalletDescriptor`, and `sendTransfer` permission approvals, checks both privacy disclosures, verifies that the exported descriptor has no CT wrapper or private key material, exercises descriptor-format/type rejection and the descriptor-change event, checks the UTXO cross-chain filter and explicit memo rejection, reloads the dapp, checks origin isolation through `127.0.0.1` versus `localhost`, observes connection events, and disconnects.

The automated suite does not perform a successful balance or UTXO scan, or broadcast a transaction, because that would make CI depend on a live Liquid service and funded wallet. Descriptor projection is local and is exercised successfully. The suite verifies that an unconnected origin receives `4100` for every implemented application RPC. Network-backed successful requests remain available for manual testing with any initialized wallet.
