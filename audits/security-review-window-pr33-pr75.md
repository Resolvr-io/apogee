...
- **Method:** window review, not a per-PR re-review. Diffstat triage of all ~35
  ...
  #63 (Liquid wallet RPC contract — the validation layer), #71 (provider
  authorization rewrite), #47 (SW tab handling), #56 (build guard), and the two
  direct commits. Everything else (#42–#48, #50–#56, #58, #62, #64–#67, #70,
  #73, #75) confirmed UI/copy/docs/display-only at diffstat and spot-check level;
  #57 is CI workflow config (review-model bump), inert at runtime. PR numbers in
  the #33–#75 range appearing in neither bucket (#35–#39, #59, #60, #68, #69,
  #72, #74) never merged to `main` — they are open, draft, closed, or issues.
...
| Scanner fallback (#52) | `scanner/detect.ts`, `package.json:21` | **Safe; one supply-chain note (finding 6).** `jsqr` is pure JS — verified against the installed package, not just its own README: the bundled `dist/jsQR.js` (10,101 lines) has zero references to fetch/XHR/WebSocket/Worker/importScripts/WebAssembly or the DOM, and the package declares no runtime dependencies. No CSP change. Native `BarcodeDetector` still handles Chromium; jsQR sees frames only on Firefox. |
| Provider input validation (#63, exercised by #71) | `provider/liquid-rpc-validation.ts` (landed in #63 with `liquid-rpc.ts` / `liquid-rpc-errors.ts`), `provider/liquid-browser-provider-validation.ts` (#71) | **Safe.** Page-supplied requests are re-parsed at the trusted boundary into fresh objects; unknown fields dropped. Every identifier is matched against an **anchored** regex — asset ids must match `^bip122:[0-9a-f]{32}\/elip144:[0-9a-f]{64}$` — so the page-controlled hex that later indexes `sync.balance` can never be `__proto__`/`constructor` (the object-index class this repo has hit twice before). Lifecycle methods cannot be requested as permissions; duplicate grants rejected. |
...
| What crosses to the page (#71) | `provider/liquid-provider.ts`, `provider/liquid-browser-provider.ts`, `provider/liquid-browser-provider-runtime.ts`, `content/content.ts`, `engine/engine-core.ts:1159` | **No key material.** `ProviderAccount` is network + fingerprint + signerKind; the descriptor (xpub + SLIP-77 blinding key) is explicitly kept out. `walletIdentity` exposes chainId + DWID (a hash) + policy asset only. The discovery announce path (`liquid-browser-provider-runtime.ts:36-41` — any page script can dispatch `liquid:requestProvider` and receive the `liquid:announceProvider` detail) carries no wallet state pre-approval: the announced detail is provider metadata plus the request/on/off surface, and every method behind it routes through the SW's authorization. Connection-changed events fan out via `tabs.sendMessage` to all tabs, but the content script forwards to the page only on `sender.id === runtime.id` **and** `message.origin === window.location.origin` — a cross-origin tab's page never sees another origin's connection snapshot. |
| Swap signing (#33/#34/#41) | `engine/engine-core.ts:943-989`, `sideswap/orchestrator.ts`, `background/index.ts:740-790` | **Safe.** Verify-then-sign is **atomic inside one engine op** (`signSwapPset`): the dealer PSET is gated by `verifyDealerPset` first, and the `lwk.Signer` is constructed only after the gate passes, so the mnemonic stays out of memory on the reject path. This closes the #32 audit's "no binding between verify and sign" wiring prerequisite. The #33 `addDetails` fix is load-bearing: without it `psetDetails` returned empty balances and every check passed trivially. The mnemonic flows SW → offscreen only; the SideSwap socket carries quote ids and PSETs, never key material (verified: no `mnemonic` reference in `sideswap/client.ts`). Password step-up (#41) applies send's exact gate to swap when auto-lock is "Never". |
...
1. **Legacy migration pins whichever wallet is active at first post-upgrade call.**
   `migrateLegacyConnection` (`background/index.ts:1068-1082`) converts a legacy
...
4. **`parseConfidentialTransaction` is a passthrough** — an object-shape check
   (`record()` rejects non-objects, arrays, null), then `{ ...params }` with no
   field validation (`liquid-rpc-validation.ts:257-260`). Unreachable today:
